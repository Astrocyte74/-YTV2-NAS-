...
- Options UI: `/zoptions` (`/zopts`) opens a control panel that shows current settings + last prompt (or use `/zopts <prompt>` to set the stored prompt without generating). Use buttons to set style (rotating), quality presets, prompt type + “Generate Prompt”, LoRA (rotating, includes None), enhance the current prompt, and Generate using the stored prompt. Advanced Settings exposes resolution/steps overrides plus queue-only and queue management.
